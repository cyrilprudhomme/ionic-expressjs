...
//   }
// })
app.post('/projects', insertToMongo);
...
async function insertToMongo(req,res){
  ...
  try {
    await mongoClient.connect();
    const exist = await mongoClient.db("app").collection(collection).findOne({name: req.body.name})
    if (!exist) {
      const result = await mongoClient.db("app").collection(collection).insertOne(req.body);
      console.log("Project ajouté " + result.insertedId)
      req.body.message = "Project ajouté " + result.insertedId
      res.status(200).json({message: req.body.message})
    } else {
